...
export default function ClientHomemember() {
  ...
  const filteredBuku = judulBuku.filter((judul) =>
    judul.toLowerCase().includes(searchQuery.toLowerCase())
  )

  // Simulasi loading saat awal render
  useEffect(() => {
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-2">
            Temukan dan pinjam buku favoritmu hari ini
          </p>
        </div>
          ...
          {(searchQuery === "" || filteredBuku.length > 0) && (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 pb-16">
              {filteredBuku.map((judul, i) => {
                const originalIndex = judulBuku.findIndex((j) => j === judul)
